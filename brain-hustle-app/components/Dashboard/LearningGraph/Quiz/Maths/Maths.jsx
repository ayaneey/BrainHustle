...
import mathQuestions from "./questions.json";
...
const Maths = () => {
	...
	// Topics organized by tier - using actual data from math-questions.json
	const topicsByTier = {
		Foundation: [
			"Number & Calculations",
			"Fractions & Decimals",
	...
	const handleTopicSelect = (topic) => {
		...
		const topicData = mathQuestions.find(
	...
	const getTimerColor = () => {
		if (timeLeft > 30) return "text-white bg-emerald-500/90";
		if (timeLeft > 15) return "text-white bg-amber-500/90";
	...
	const getQuestionCountForDisplay = (topic) => {
		if (!mathQuestions || !Array.isArray(mathQuestions)) return 20;
		const topicData = mathQuestions.find((t) => t.topic === topic);
		return topicData?.questions?.length || 20;
	};

	// Safety check for mathQuestions
	if (!mathQuestions || !Array.isArray(mathQuestions)) {
		return (
			<div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50 px-3 sm-phone:px-4">
				<div className="text-center p-4 md-phone:p-6 sm-tablet:p-8">
					...
					<p className="text-gray-600 text-sm md-phone:text-sm sm-tablet:text-base">
						Please check that math-questions.json is properly formatted.
					</p>
				</div>
			</div>
		);
	...
	return (
		<div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50 relative overflow-hidden">
			...
			<div className="absolute inset-0 overflow-hidden">
				<div className="absolute -top-4 -left-4 w-24 h-24 sm-phone:w-32 sm-phone:h-32 md-phone:w-36 md-phone:h-36 sm-tablet:w-48 sm-tablet:h-48 lg-tablet:w-72 lg-tablet:h-72 bg-gradient-to-br from-orange-200 to-amber-300 rounded-full opacity-20 animate-pulse"></div>
				<div
					className="absolute top-1/4 -right-4 w-32 h-32 sm-phone:w-40 sm-phone:h-40 md-phone:w-48 md-phone:h-48 sm-tablet:w-64 sm-tablet:h-64 lg-tablet:w-96 lg-tablet:h-96 bg-gradient-to-br from-yellow-200 to-orange-300 rounded-full opacity-20 animate-pulse"
					style={{ animationDelay: "2s" }}
				></div>
				<div
					className="absolute -bottom-4 left-1/4 w-28 h-28 sm-phone:w-36 sm-phone:h-36 md-phone:w-44 md-phone:h-44 sm-tablet:w-56 sm-tablet:h-56 lg-tablet:w-80 lg-tablet:h-80 bg-gradient-to-br from-amber-200 to-yellow-300 rounded-full opacity-20 animate-pulse"
			...
			<div className="container mx-auto px-3 sm-phone:px-4 md-phone:px-5 sm-tablet:px-6 py-4 sm-phone:py-6 md-phone:py-8 sm-tablet:py-10 lg-tablet:py-12 relative z-10">
				...
				<div className="text-center mb-6 sm-phone:mb-8 md-phone:mb-10 sm-tablet:mb-12 lg-tablet:mb-16">
					<div className="flex flex-col sm-tablet:flex-row items-center justify-center gap-3 sm-phone:gap-4 md-phone:gap-5 sm-tablet:gap-6 mb-4 sm-phone:mb-6 md-phone:mb-7 sm-tablet:mb-8">
						<div className="relative">
							<div className="absolute inset-0 bg-gradient-to-r from-orange-400 to-yellow-500 rounded-xl sm-phone:rounded-2xl sm-tablet:rounded-3xl blur-xl opacity-40 animate-pulse"></div>
							<div className="relative p-2 sm-phone:p-3 md-phone:p-4 sm-tablet:p-5 lg-tablet:p-6 bg-gradient-to-r from-orange-500 to-yellow-600 rounded-xl sm-phone:rounded-2xl sm-tablet:rounded-3xl shadow-xl">
								<Calculator className="w-5 h-5 sm-phone:w-6 sm-phone:h-6 md-phone:w-7 md-phone:h-7 sm-tablet:w-8 sm-tablet:h-8 lg-tablet:w-10 lg-tablet:h-10 text-white" />
							</div>
						</div>
						<div className="text-center sm-tablet:text-left">
							<h1 className="text-2xl sm-phone:text-3xl md-phone:text-4xl lg-phone:text-4xl xl-phone:text-5xl sm-tablet:text-5xl lg-tablet:text-6xl font-black bg-gradient-to-r from-orange-600 via-amber-600 to-yellow-600 bg-clip-text text-transparent leading-tight">
								AQA GCSE Maths
							</h1>
							<p className="text-amber-600 text-xs sm-phone:text-sm md-phone:text-base sm-tablet:text-lg lg-tablet:text-xl font-semibold mt-1 sm-phone:mt-2 sm-tablet:mt-3 flex items-center justify-center sm-tablet:justify-start gap-2">
				...
				{!selectedTier && (
					<div className="max-w-5xl mx-auto">
						<div className="text-center mb-6 sm-phone:mb-8 md-phone:mb-10 sm-tablet:mb-12 lg-tablet:mb-16">
							...
							{/* Tier Cards */}
							<div className="grid grid-cols-1 sm-tablet:grid-cols-2 gap-6 sm-phone:gap-8 md-phone:gap-10 max-w-4xl mx-auto">
								{tiers.map((tier, index) => (
									<div
										key={tier.name}
										onClick={() => handleTierSelect(tier.name)}
										className={`group relative bg-gradient-to-br ${tier.bgGradient} rounded-xl sm-phone:rounded-2xl sm-tablet:rounded-3xl p-6 sm-phone:p-8 md-phone:p-10 lg-tablet:p-12 shadow-xl border-2 border-white transition-all duration-500 transform hover:scale-105 cursor-pointer hover:shadow-2xl`}
									>
										<div className="relative z-10">
											<div className="flex items-center justify-between mb-6 sm-phone:mb-8">
												<div
													className={`p-4 sm-phone:p-5 md-phone:p-6 rounded-xl sm-phone:rounded-2xl bg-gradient-to-r ${tier.gradient} shadow-lg transform group-hover:rotate-12 transition-transform duration-300`}
												>
													<div className="text-2xl sm-phone:text-3xl md-phone:text-4xl lg-tablet:text-5xl">
														{tier.icon}
													</div>
												</div>
												<div className="bg-white/80 backdrop-blur-sm px-2 py-1 sm-phone:px-3 sm-phone:py-2 md-phone:px-4 md-phone:py-2 rounded-lg sm-phone:rounded-xl lg-tablet:rounded-2xl text-gray-700 font-bold border border-gray-200 shadow-sm text-xs sm-phone:text-sm md-phone:text-base">
													Multiple topics
												</div>
											</div>

											<h3 className="text-xl sm-phone:text-2xl md-phone:text-3xl lg-phone:text-3xl xl-phone:text-4xl sm-tablet:text-4xl lg-tablet:text-5xl font-bold mb-3 sm-phone:mb-4 text-gray-800 group-hover:text-gray-900 transition-colors">
												{tier.name}
											</h3>
											<p className="text-gray-600 text-sm sm-phone:text-base md-phone:text-lg sm-tablet:text-lg leading-relaxed mb-6 sm-phone:mb-8">
												{tier.description}
											</p>

											<div className="flex items-center text-gray-700 text-base sm-phone:text-lg md-phone:text-xl sm-tablet:text-xl font-bold group-hover:text-gray-900 transition-colors">
												<span>Start Journey</span>
												<svg
													className="w-5 h-5 sm-phone:w-6 sm-phone:h-6 md-phone:w-7 md-phone:h-7 ml-2 sm-phone:ml-3 transform group-hover:translate-x-2 transition-transform duration-300"
													fill="none"
													stroke="currentColor"
													viewBox="0 0 24 24"
												>
				...
				{selectedTier && !selectedTopic && (
					<div className="max-w-6xl mx-auto">
						<div className="text-center mb-6 sm-phone:mb-8 md-phone:mb-10 sm-tablet:mb-12 lg-tablet:mb-16">
							<div className="flex flex-col sm-tablet:flex-row items-center justify-between mb-4 sm-phone:mb-6 md-phone:mb-8">
								<div className="flex items-center gap-4 mb-4 sm-tablet:mb-0">
									<div className="text-sm sm-phone:text-base md-phone:text-lg sm-tablet:text-lg text-gray-600">
								...
								<button
									...
									className="p-2 sm-phone:p-3 md-phone:p-4 rounded-xl sm-phone:rounded-2xl hover:bg-gray-100 transition-all duration-300 transform hover:scale-110"
									aria-label="Back to tier selection"
								>
									<ArrowLeft className="w-4 h-4 sm-phone:w-5 sm-phone:h-5 md-phone:w-6 md-phone:h-6 sm-tablet:w-7 sm-tablet:h-7 text-gray-600" />
								</button>
							</div>
											...
											<div className="relative z-10">
												<div className="flex items-start justify-between mb-3 sm-phone:mb-4 md-phone:mb-5 sm-tablet:mb-6 lg-tablet:mb-8">
													...
													<div className="bg-white/80 backdrop-blur-sm px-2 py-1 sm-phone:px-2 sm-phone:py-1 md-phone:px-3 md-phone:py-1 sm-tablet:px-3 sm-tablet:py-2 lg-tablet:px-4 lg-tablet:py-2 rounded-lg sm-phone:rounded-xl sm-tablet:rounded-2xl text-gray-700 font-bold border border-gray-200 shadow-sm text-xs sm-phone:text-xs md-phone:text-sm sm-tablet:text-sm">
														{getQuestionCountForDisplay(topic)} questions
													</div>
												</div>
				...
				{selectedTopic && !showResults && currentQ && (
					<div className="max-w-5xl mx-auto">
						<div className="bg-white/90 backdrop-blur-sm rounded-xl sm-phone:rounded-2xl sm-tablet:rounded-3xl shadow-2xl border border-white overflow-hidden">
							{/* Quiz Header */}
							<div className="bg-gradient-to-r from-orange-500 via-amber-600 to-yellow-600 p-3 sm-phone:p-4 md-phone:p-5 sm-tablet:p-6 lg-tablet:p-8 xl-tablet:p-10 text-white relative overflow-hidden">
								...
								<div className="relative z-10">
									<div className="flex flex-col sm-tablet:flex-row items-start sm-tablet:items-center justify-between gap-3 sm-phone:gap-4 md-phone:gap-5 sm-tablet:gap-6 lg-tablet:gap-8 mb-4 sm-phone:mb-5 md-phone:mb-6 sm-tablet:mb-8">
										...
										<div className="flex flex-wrap items-center gap-2 sm-phone:gap-3 md-phone:gap-4 lg-tablet:gap-8">
											...
											<div className="px-2 py-1 sm-phone:px-3 sm-phone:py-2 md-phone:px-4 md-phone:py-2 sm-tablet:px-4 sm-tablet:py-2 lg-tablet:px-6 lg-tablet:py-3 rounded-lg sm-phone:rounded-xl sm-tablet:rounded-2xl font-bold border-2 bg-white/80 text-gray-700 text-xs sm-phone:text-sm md-phone:text-sm sm-tablet:text-base">
												{selectedTier}
											</div>
										</div>
									</div>
									...
									<div>
										...
										<p className="text-orange-100 text-xs sm-phone:text-sm md-phone:text-sm sm-tablet:text-base lg-tablet:text-lg xl-tablet:text-xl">
							...
							<div className="w-full bg-gray-200 h-2 sm-tablet:h-3 lg-tablet:h-4 relative overflow-hidden">
								<div
									className="bg-gradient-to-r from-orange-400 via-amber-500 to-yellow-500 h-2 sm-tablet:h-3 lg-tablet:h-4 transition-all duration-1000 ease-out relative"
						...
						<div className="bg-white/90 backdrop-blur-sm rounded-xl sm-phone:rounded-2xl sm-tablet:rounded-3xl shadow-2xl border border-white overflow-hidden">
							...
							<div className="p-3 sm-phone:p-4 md-phone:p-5 sm-tablet:p-6 lg-tablet:p-8 xl-tablet:p-12">
								...
								<div className="flex flex-col sm-tablet:flex-row gap-3 sm-phone:gap-4 md-phone:gap-5 sm-tablet:gap-6 lg-tablet:gap-8 justify-center">
									<button
										onClick={goBackToTopics}
										className="group relative px-4 py-3 sm-phone:px-5 sm-phone:py-4 md-phone:px-6 md-phone:py-4 sm-tablet:px-8 sm-tablet:py-5 lg-tablet:px-10 lg-tablet:py-6 xl-tablet:px-12 xl-tablet:py-6 bg-gradient-to-r from-orange-500 via-amber-600 to-yellow-600 text-white rounded-xl sm-phone:rounded-2xl sm-tablet:rounded-3xl font-bold text-sm sm-phone:text-base md-phone:text-base sm-tablet:text-lg lg-tablet:text-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-110 flex items-center justify-center gap-2 sm-phone:gap-3 overflow-hidden shadow-xl"
									>
